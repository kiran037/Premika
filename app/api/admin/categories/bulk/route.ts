...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      return NextResponse.json({ success: false, message: "No categories selected" }, { status: 400 });
    }
    ...
    await CategoryService.bulkAdminCategoryAction(ids, action);
    return NextResponse.json({ success: true, message: `Bulk ${action} executed successfully` }, { status: 200 });
  } catch (err: any) {
    return NextResponse.json({ success: false, message: err.message || "Bulk action failed" }, { status: 400 });
  }
}
